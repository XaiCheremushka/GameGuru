...
class GenreModel {
    ...
    public function create($data) {
        ...
        if (!$stmt->execute()) {
            ...
            throw new \Exception('Ошибка при создании жанра: ' . ($errorInfo[2] ?? 'Неизвестная ошибка'));
        }
        
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$result) {
            throw new \Exception('Не удалось получить ID созданного жанра');
        }
        return $result['id'];
    }
    ...
    public function update($id, $data) {
        ...
        if (!$stmt->execute()) {
            $errorInfo = $stmt->errorInfo();
            throw new \Exception('Ошибка при обновлении жанра: ' . ($errorInfo[2] ?? 'Неизвестная ошибка'));
        }
